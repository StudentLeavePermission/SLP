...
const UpdatePengajuanMahasiswa = () => {
    ...
    const getLeaveRequests = async () => {
        try {
            ...
            getAllLeaveRequests(response.data.mahasiswa[0].Data_Mahasiswa.id, response.data.data.Jenis_Izin, response.data.data.Tanggal_Pengajuan, response.data.jadwal)
            console.log('coba', selectedjadwalcoba)
        } catch (error) {
            console.error('Error fetching data:', error);
        }
    }
    const getAllLeaveRequests = async (id, jenis, tanggal, dataJamPelajaran) => {
        try {
            ...
            pengajuan = pengajuan.filter(item => item.Jenis_Izin === jenis && item.Tanggal_Pengajuan === tanggal);
            ...
            while (pengajuan.length > 0) {
                ...
                setHari(getDayName(formattedData.date))
